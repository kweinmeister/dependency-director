...
@pytest.mark.asyncio
async def test_get_repositories_filters_forks(github_token: str) -> None:
    """Verify that get_repositories filters out forked repositories."""
    mock_repos_page1 = [
...
async def test_get_repositories_pagination(github_token: str) -> None:
    ...
    page_2_data = [{"name": "repo-2", "fork": False}]
    page_3_data: list[dict[str, Any]] = []
    with patch("httpx.AsyncClient.get") as mock_get:
        mock_user = MagicMock()
        mock_user.status_code = 200
        mock_user.json.return_value = {"login": "not-test-owner"}
...
async def test_run_agent_concurrency(
    ...
    """Verify that run_agent limits concurrent executions to the configured value."""
    mock_get_repos.return_value = ["test-owner/repo-1", "test-owner/repo-2", "test-owner/repo-3"]
...
@pytest.mark.asyncio
async def test_get_repositories_http_error(github_token: str) -> None:
    """Verify that get_repositories handles HTTP errors gracefully by raising an exception."""
    with patch("httpx.AsyncClient.get") as mock_get:
        mock_user = MagicMock()
        mock_user.status_code = 200
        mock_user.json.return_value = {"login": "not-test-owner"}
        ...
        with pytest.raises(httpx_mod.HTTPStatusError):
            await get_repositories(owner="test-owner", token=github_token)


@pytest.mark.asyncio
async def test_get_repositories_empty_token() -> None:
    """Verify that get_repositories executes without authentication when token is empty."""
...
@pytest.mark.asyncio
async def test_get_repositories_no_repos(github_token: str) -> None:
    """Verify that get_repositories returns an empty list if no repositories are found."""
    with patch("httpx.AsyncClient.get") as mock_get:
        mock_user = MagicMock()
        mock_user.status_code = 200
        mock_user.json.return_value = {"login": "not-test-owner"}
...
@pytest.mark.asyncio
async def test_get_repositories_falls_back_to_orgs_on_404(github_token: str) -> None:
    """Verify get_repositories falls back to org repos if the user repos API returns 404."""
    mock_user = MagicMock()
    mock_user.status_code = 200
    ...
    mock_org_page1.json.return_value = [{"name": "org-repo-1", "fork": False}, {"name": "org-repo-2", "fork": True}]
    ...
    with patch("httpx.AsyncClient.get") as mock_get:
        ...
        repos = await get_repositories(owner="my-org", token=github_token)
...
@pytest.mark.asyncio
async def test_get_repositories_invalid_owner(github_token: str) -> None:
    """Verify that get_repositories raises a ValueError for malformed owner names."""
    with pytest.raises(ValueError, match="Invalid owner name"):
        await get_repositories(owner="../invalid-owner", token=github_token)
...
async def test_run_agent_worker_error_isolates(
    ...
    """Verify that a worker failure on one repository does not abort execution for others."""
...
@pytest.mark.asyncio
@patch("dependency_director.main.get_repositories", new_callable=AsyncMock)
async def test_run_agent_no_repos_found(mock_get_repos: MagicMock, github_token: str) -> None:
    """Verify that run_agent exits cleanly with a warning when no repositories are found."""
...
@pytest.mark.asyncio
async def test_run_agent_no_api_key_no_vertex_exits(github_token: str) -> None:
...
@pytest.mark.asyncio
async def test_run_agent_vertex_missing_project_exits(github_token: str) -> None:
...
@patch("dependency_director.main.run_agent_for_repo", new_callable=AsyncMock)
@patch("dependency_director.main.get_repositories", new_callable=AsyncMock)
async def test_run_agent_no_github_token_warns(mock_get_repos: MagicMock, mock_run_agent_for_repo: MagicMock) -> None:
...
@pytest.mark.asyncio
@patch("dependency_director.main.get_repositories", new_callable=AsyncMock)
async def test_run_agent_get_repos_exception(mock_get_repos: MagicMock, github_token: str) -> None:
...
@pytest.mark.asyncio
async def test_run_agent_verify_all_no_sandbox_incompatible(github_token: str) -> None:
...
@pytest.mark.asyncio
async def test_get_repositories_authenticated_owner(github_token: str) -> None:
    """Verify that get_repositories fetches repositories for the authenticated user."""
    owner = "test-owner"
    token = github_token
    ...
    mock_repos_response.json.return_value = [{"name": "repo-priv", "fork": False}, {"name": "repo-fork", "fork": True}]
...
@pytest.mark.asyncio
async def test_get_repositories_other_owner(github_token: str) -> None:
    """Verify that get_repositories fetches public repositories for other target owners."""
    owner = "other-owner"
    token = github_token
    mock_user_response = MagicMock()
    mock_user_response.status_code = 200
    ...
    mock_repos_response = MagicMock()
    mock_repos_response.status_code = 200
    mock_repos_response.json.return_value = [{"name": "repo-pub", "fork": False}]
    mock_repos_empty = MagicMock()
    mock_repos_empty.status_code = 200
    mock_repos_empty.json.return_value = []
    calls: list[Any] = []
...
@pytest.mark.asyncio
async def test_get_repositories_user_endpoint_fails(github_token: str) -> None:
    """Verify get_repositories falls back to org endpoint if user endpoint fails with HTTP error."""
    owner = "test-owner"
    token = github_token
    mock_user_response = MagicMock()
    ...
    def side_effect(url: Any, *_args: Any, **_kwargs: Any) -> Any:
        calls.append(url)
        if "api.github.com/user" in str(url) and "repos" not in str(url):
            raise httpx_mod.HTTPStatusError(message="Unauthorized", request=MagicMock(), response=mock_user_response)
        if f"api.github.com/users/{owner}/repos" in str(url):
            if len(calls) == 2:
                return mock_repos_response
            return mock_repos_empty
...
@pytest.mark.asyncio
async def test_get_repositories_scope_error_propagates(github_token: str) -> None:
    """Verify token scope errors (such as HTTP 403) are propagated up correctly."""
    owner = "test-owner"
    token = github_token

    def side_effect(_url: Any, *_args: Any, **_kwargs: Any) -> Never:
        msg = "Unauthorized/Bad Scope"
        raise GitHubAuthenticationError(msg)

    with (
        patch("httpx.AsyncClient.get", side_effect=side_effect),
        pytest.raises(GitHubAuthenticationError, match="Unauthorized/Bad Scope"),
    ):
        await get_repositories(owner=owner, token=token)


@pytest.mark.asyncio
async def test_get_repositories_user_repos_404_fallback(github_token: str) -> None:
    """Verify fallback behavior when fetching user repos results in 404."""
    owner = "test-owner"
    token = github_token
    mock_user_response = MagicMock()
    mock_user_response.status_code = 200
    ...
    def side_effect(url: Any, *_args: Any, **_kwargs: Any) -> Any:
        ...
        if "api.github.com/user" in str(url) and "repos" not in str(url):
            return mock_user_response
        if "api.github.com/user/repos" in str(url):
            raise httpx_mod.HTTPStatusError(message="Not Found", request=MagicMock(), response=mock_user_repos_404)
        if f"api.github.com/users/{owner}/repos" in str(url):
            if len(calls) == 3:
...
@pytest.mark.asyncio
async def test_get_repositories_token_caching(github_token: str) -> None:
    """Verify that GitHubClient caches and reuses client headers based on the token."""
    owner = "test-owner"
    token = github_token
    mock_user_response = MagicMock()
    mock_user_response.status_code = 200
