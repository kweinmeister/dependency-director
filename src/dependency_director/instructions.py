    ...
    *,
    ...
    if no_sandbox:
        ...
        workflow_content = (
            ...
            "so never pull them for a PR a red base has already excused, and read them at "
            "most once per distinct failure: for a RED PR whose failing check names are ones "
            "you have already read logs for this run, skip 'get_pr_workflow_run_logs' and go "
            "straight to reproducing locally. Read that PR's logs only if the local run does "
            "not reproduce the failure or shows a different error than the one you diagnosed. "
