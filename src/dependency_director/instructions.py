...
def get_system_instructions(
    ...
    *,
    ...
    base_health_check = (
        "FIRST, before pulling any logs, call 'get_branch_ci_status(owner, repo, branch)' with branch set to "
    ...
    if no_sandbox:
        ...
        workflow_content = (
            ...
            f"   - RED: {base_health_check}"
            "Only once the base is cleared does the PR's own failure need investigating: "
            "get logs via 'get_pr_workflow_run_logs', then 'get_pr_diff' and 'get_pr_files' "
            "to understand scope before cloning. Logs are the most expensive read in a run, "
            "so never pull them for a PR a red base has already excused. "
            "Read what the logs actually blame before you clone. A check that died from a "
            "runner error — the action hit a turn limit, a rate or quota limit, ran out of "
            "memory, timed out, or could not authenticate — never reached this repository's "
            "code, so no local run can reproduce it and no edit of yours can fix it. Do NOT "
            "clone such a PR: log '⚠ #<n> not fixed: <check> failed from a runner error, not "
            "the dependency update' and move to the next one. "
            "Diagnose each distinct failure at most once: for a RED PR whose failing check "
            "names are ones you have already read logs for this run, skip "
            "'get_pr_workflow_run_logs'. If that earlier diagnosis put the cause outside the "
            "dependency update, skip the clone too and log the same outcome for this PR — "
            "re-running clone, sync and tests to re-confirm a verdict you already hold is the "
            "most expensive way to learn nothing. Clone and reproduce again only where the "
            "earlier diagnosis was a real code failure, and re-read that PR's logs only if "
            "the local run does not reproduce the failure or shows a different error than the "
            "one you diagnosed. "
            ...
            "you did not make. Claim 'did not reproduce locally' only for a PR you actually ran "
            "the tests on; where you skipped the clone, give the reason that applies instead. "
            f"Otherwise fix, verify, and: {fix_strategy}\n"
        ...
        sections.append(
            types.SystemInstructionSection(
                ...
                content=(
                    ...
                    "- FIRST, before cloning, call 'find_open_pr_for_branch(owner, repo, "
                    "'dependency-director/fix-base-<base_ref>')'. If it reports an open PR, "
                    "an earlier run already fixed this base and the PR is awaiting review: "
                    "log '⚠ #<n> not fixed: blocked on base fix PR #<k>' for every RED PR on "
                    "that base and move on. Do NOT clone, rebuild, or re-push that branch.\n"
                    ...
                    "- 'uv sync' and its equivalents rewrite lockfiles (uv.lock, "
                    "package-lock.json, Cargo.lock) as a side effect. Restore them before "
                    "committing unless the failing check is about the lockfile itself.\n"
        ...
        sections.append(
            types.SystemInstructionSection(
                ...
                content=(
                    ...
                    "A push refused with \"Denied by policy 'dry_run_block_push...'\" is the "
                    "expected outcome of this mode, not a failure: treat it as a successful "
                    "simulated push, do not retry it, and do not report it. "
    ...
    sections.append(
        types.SystemInstructionSection(
            ...
            content=(
                ...
                "  4. Final summary of all processed PRs as a markdown bullet list, one bullet "
                "per PR. Never a table: rows repeating a near-identical reason trip the "
                "looping-content detector and the run dies mid-render.\n"
